...
private:
  BSPNode *_root;
  vector<Segment *>
      _segments; // these are for storing segments and points when _root is null
...
void BSPSortMethod::sortPrimitives(std::vector<PtrPrimitive> &primitive_tab,
  ...
  vector<PtrPrimitive> segments_and_points; // Store segments and points for
                                            // pass 2, because polygons are
                                            // deleted by the insertion and can
...
void BSPNode::Classify(Polygone *P, Polygone *&moins_, Polygone *&plus_) {
  ...
  for (int i = 0; i < n; i++) {
    double Z =
        P->vertex(i).x() * a + P->vertex(i).y() * b + P->vertex(i).z() * c - d;
