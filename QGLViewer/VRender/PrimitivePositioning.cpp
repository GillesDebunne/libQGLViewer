...
int PrimitivePositioning::computeRelativePosition(const Primitive *p1,
  ...
  // 1 - check if bounding boxes are disjoint. In such a case, a rapid answer is
  // possible.
  ...
  if (p1->nbVertices() >= 3)
    ...
    return inverseRP(computeRelativePosition(dynamic_cast<const Polygone *>(p2),
                                             dynamic_cast<const Point *>(p1)));
  else if (p2->nbVertices() == 2)
...
// lui-meme bords inclus. Pour EPS<0, des points interieurs pres de la frontiere
// sont declares exterieurs.  Plus I_EPS est grand, plus l'ensemble des points
...
void PrimitivePositioning::getsigns(const Primitive *P, const NVector3 &v,
                                    ...
                                    vector<double> &zvals, int &Smin, int &Smax,
                                    double I_EPS) {
  if (P == NULL)
...
void PrimitivePositioning::split(Polygone *P, const NVector3 &v, double C,
  ...
  for (size_t j = 1; j <= n; j++) {
    ...
    if (sign == prev_sign) {
      ...
      //  Il faut effectuer le calcul en utilisant les memes valeurs que pour le
      //  calcul des signes,
