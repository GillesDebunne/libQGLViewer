...
void ParserGL::parseFeedbackBuffer(GLfloat *buffer, int size,
  ...
  ParserUtils::ComputeBufferBB(size, buffer, _xmin, _xmax, _ymin, _ymax, _zmin,
                               _zmax);
...
PtrPrimitive ParserUtils::checkPolygon(Polygone *&P) {
  if (P->nbVertices() != 3) {
    cout << "unexpected case: Polygon with " << P->nbVertices() << " vertices !"
         << endl;
    delete P;
    return NULL;
  }
...
void ParserUtils::NormalizePrimitiveCoordinates(GLfloat *&loc, GLfloat MaxSize,
  ...
  case GL_LINE_TOKEN: {
    ...
    loc +=
        2 *
        size; /* Each vertex element in the feedback buffer is size GLfloats. */
    break;
  }
  case GL_POLYGON_TOKEN: {
    nvertices = int(*loc);
