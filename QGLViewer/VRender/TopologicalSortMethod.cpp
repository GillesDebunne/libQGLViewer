...
public:
  ...
  static void checkAndAddEdgeToGraph(size_t a, size_t b,
                                     vector<vector<size_t>> &precedence_graph);
...
void TopologicalSortMethod::sortPrimitives(vector<PtrPrimitive> &primitive_tab,
...
void TopologicalSortUtils::recursFindNeighbors(
  ...
  // No refinment either because it could not be possible, or because the number
  // of primitives is below the predefined limit.
...
void TopologicalSortUtils::recursTopologicalSort(
  ...
  for (size_t j = 0; j < precedence_graph[indx].size(); ++j) {
    ...
    if (!already_visited[precedence_graph[indx][j]]) {
      if (!already_rendered[precedence_graph[indx][j]]) {
        recursTopologicalSort(
            precedence_graph, primitive_tab, already_rendered, already_visited,
            new_pr_tab, precedence_graph[indx][j], ancestors,
            ancestors_backward_index, nb_cycles, vparams, info_cnt, nbrendered);
      ...
      // are successively split by a chosen splitting plane and the precendence
      // graph is updated at the same time by re-computing primitive precedence.
      // As soon as the cycle is broken, the algorithm stops and calls
      // recursively calls on the new precedence graph. This necessarily happens
      // because of the BSP-node nature of the current set of primitives.
      ...
      for (size_t i3 = (size_t)cycle_beginning_index + 1;
        ...
        if (ancestors[i3] != (size_t)split_prim_indx) {
          ...
          PrimitivePositioning::splitPrimitive(
              primitive_tab[ancestors[i3]], normal, c, prim_upper, prim_lower);
            ...
            if (l != (size_t)lower_indx)
              for (size_t k = 0; k < precedence_graph[l].size(); ++k)
                if (precedence_graph[l][k] == ancestors[i3]) {
                  ...
                  // It can not be Lower, because it was upper from the original
                  // primitive, but it is not necessary upper any longer because
                  // of the split.
                  ...
                  if (prp2 & PrimitivePositioning::Upper) {
                    ...
                    cout << " > " << endl;
#endif
                    if (l == (size_t)ancestors[i3 - 1]) // The index is the same
                                                        // => nothing to change.
