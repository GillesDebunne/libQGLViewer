...
preserved on all copies. The intellectual property rights of the algorithms used
reside with the University of Manchester Advanced Interfaces Group.
...
void gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
  ...
  edge_node *edge = 0, *prev_edge = 0, *next_edge = 0, *succ_edge = 0, *e0 = 0,
            *e1 = 0;
  edge_node *aet = NULL, *c_heap = NULL, *s_heap = NULL;
  lmt_node *lmt = NULL, *local_min = 0;
  polygon_node *out_poly = NULL, *p = 0, *q = 0, *poly = 0, *npoly = 0,
               *cf = NULL;
      ...
      if (exists[CLIP] || exists[SUBJ]) {
        ...
        case GPC_XOR:
          ...
          tr = (parity[CLIP] ^ (horiz[CLIP] != NH)) ^
               (parity[SUBJ] ^ (horiz[SUBJ] != NH));
          tl =
              (parity[CLIP] ^ (horiz[CLIP] != NH) ^ edge->bundle[BELOW][CLIP]) ^
              (parity[SUBJ] ^ (horiz[SUBJ] != NH) ^ edge->bundle[BELOW][SUBJ]);
          break;
        case GPC_UNION:
          contributing =
