...
#include <iostream>
  ...
  {
  public:
    inline argstream(int argc,char** argv,bool debug=false);
    inline argstream(const char* c,bool debug=false);
    template<class T>
    friend argstream& operator>>(argstream& s,const ValueHolder<T>& v);
  ...
  //************************************************************
  inline OptionHolder::OptionHolder(const char s,
			     const char* l,
			     bool& b,
			     const char* desc)
    : shortName_(1,s),
  ...
  inline
  argstream::argstream(const char* c,bool debug)
    : debug_(debug),
      progName_(""),
      minusActive_(true),
      isOk_(true)
...
// Local Variables:
// ff-search-directories: ("../../../src/tools/")
// End:
